...
def vis_rggb(img):
    rgb = np.stack([img[:, :, 0], img[:, :, 1:3].mean(-1), img[:, :, -1]], axis=-1)
...
if __name__ == "__main__":
    ...
    data_dir = "/data2/feiran/datasets/aim_challenge_release_data"
    ...
    for i, data in enumerate(test_loader):
        ...
        print(i, noisy.shape, clean.shape, data["cam_model"], data["dgain"], data["iso"])

        noisy, clean = vis_rggb(noisy[0, 0].permute(1, 2, 0).numpy()), vis_rggb(clean[0, 0].permute(1, 2, 0).numpy())
